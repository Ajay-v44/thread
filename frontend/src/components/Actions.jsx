...
const Actions = ({ post }) => {
  ...
  const handleOnLike = async () => {
    if (!user) return toast("Error", "login to like a post", "error");
    if (liking) return;
    setliking(true);
    try {
      ...
      if (!liked) {
        ...
        setPost((prevPost) => ({
          ...prevPost,
          likes: prevPost.likes.filter((id) => id !== user._id),
        }));
      }
  ...
  const handleReply = async () => {
    ...
    try {
      ...
      setPost((prevPost) => ({
        ...prevPost,
        replies: [...prevPost.replies, user._id],
      }));
      if (res.status === 200) {
        toast("Replied", res.data.message, "success");
      }
    } catch (err) {
      console.log(err);
      toast("Error", "Somethingwent wrong", "error");
    } finally {
      onClose();
      setreply("");
    }
